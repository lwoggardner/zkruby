  ...
  module Proto
    ...
    class MultiRequest < BinData::Record
      array :requests, :read_until => lambda { element.header._type < 0 } do
        multi_header :header
        choice :request, :selection => lambda { header._type }, :onlyif => lambda { header._type > 0 } do
          create_request 1
          delete_request 2
          set_data_request 5
    ...
    class MultiResponseItem < BinData::Record
      ...
      choice :response, :selection => lambda { header._type },
        :onlyif => lambda { has_response? },
        :choices => { -1 => :error_response,  1 => :create_response, 5 => :set_data_response }
