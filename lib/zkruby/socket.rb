...
# Add some useful methods to socket
class Socket
  ...
  def self.tcp_connect_timeout(host,port,timeout = 0)
    ...
    if HAS_NONBLOCKING_CONNECT && timeout > 0
      # TODO: This is a blocking DNS lookup!!!!, possibly even a reverse lookup if host is a numeric address
  ...
  def read_timeout(timeout,maxlen,buffer = nil)
    begin
      return read_nonblock(maxlen,buffer)
