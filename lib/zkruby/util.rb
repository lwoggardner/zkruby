module ZooKeeper
  ...
  class Client
    ...
    def mkpath(path,acl=ZK::ACL_OPEN_UNSAFE,&callback)
      ...
      return mkpath(path,acl) { |*results| results }.value unless block_given?
      ...
      ( 1-path_comp.length..-1 ).each do |i|
        ...
        op = create(sub_path,"",acl) { i == -1 ? callback.call() : :true  }
    ...
    def rmpath(path,version = -1, &callback)
      ...
      return rmpath(path,version) { |*results| results }.value unless block_given?
